 ...
 *
 * CUDA demos (\p PATHTRACER_CUDA_RNG) only compile GPU-safe textures here.
 * Host-only types (shared_ptr Checkered, stb Wallpaper, Perlin Noise) stay
 * behind the guard so nvcc does not see execution-space mismatches or stb
...
public:
    ...
    Checkered(float scale, const vec3& evenColor, const vec3& oddColor)
        : Checkered(scale, std::make_shared<SolidColor>(evenColor),
                    std::make_shared<SolidColor>(oddColor)) {}
