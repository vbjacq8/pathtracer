...
/**
 * CPU counterpart of raytracer/test/cu/shaded_sphere.cu: red Lambertian sphere
 * + yellow ground, classic RTIOW camera at the origin looking down -z.
 *
 *   ./run_cpp_test.sh shaded_sphere.cpp
 *   ./run_cpp_test.sh shaded_sphere.cpp --width 400 --samples 50 --depth 10
 */
static HitablePtr shadedSphereScene() {
    auto world = std::make_shared<HitableList>();
    world->add(std::make_shared<Sphere>(vec3(0.0f, 0.0f, -1.0f), 0.5f,
                                        new Lambertian(vec3(1.0f, 0.0f, 0.0f))));
...
int main(int argc, char** argv) {
    ...
    HitablePtr world = shadedSphereScene();
