...
TEST(HitableHit, SphereHitsAtKnownDistance) {
    ...
    ASSERT_TRUE(sphere->hit(r, 0.0, 1e30, hr));
...
TEST(HitableHit, SphereMissesWhenRaySkipsObject) {
    ...
    EXPECT_FALSE(sphere->hit(r, 0.0, 1e30, hr));
}
...
TEST(HitableHit, SecondaryRayWithoutEpsCanSelfIntersectDueToFloatingPoint) {
    ...
    ASSERT_TRUE(sphere->hit(primary, 0.0, 1e30, primaryHit));
    ...
    if (sphere->hit(secondary, 0.0, 1e30, secondaryHit)) {
...
TEST(HitableHit, SecondaryRayWithHitEpsAvoidsSelfIntersection) {
    ...
    EXPECT_FALSE(sphere->hit(secondary, kHitEps, 1e30, secondaryHit))
    ...
    HitRecord farHit;
    ASSERT_TRUE(sphere->hit(inward, kHitEps, 1e30, farHit));
    EXPECT_GT(farHit.t, kHitEps);
    EXPECT_NEAR(farHit.t, 2.0, 1e-4);
}

TEST(HitableHit, BoxHitRespectsTMinBias) {
    HitablePtr box = std::make_shared<Box>(vec3(-1, -1, -1), vec3(1, 1, 1), dummyMat());
    const Ray primary(vec3(-3, 0, 0), vec3(1, 0, 0));
    HitRecord primaryHit;
    ASSERT_TRUE(box->hit(primary, 0.0, 1e30, primaryHit));
    EXPECT_NEAR(primaryHit.t, 2.0, kGeomEps);

    const Ray secondary(primaryHit.p, primaryHit.normal);
    HitRecord secondaryHit;
    EXPECT_FALSE(box->hit(secondary, kHitEps, 1e30, secondaryHit));
}
...
TEST(HitableHit, HitableListUsesPolymorphicHitThroughHitablePtr) {
    ...
    world->add(std::make_shared<Box>(vec3(5, -0.5, -0.5), vec3(6, 0.5, 0.5), dummyMat()));

    HitablePtr scene = world;
    HitRecord hr;

    // Hits the nearer sphere first.
    ASSERT_TRUE(scene->hit(Ray(vec3(-5, 0, 0), vec3(1, 0, 0)), kHitEps, 1e30, hr));
